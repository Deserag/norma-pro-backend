...
export class ClientController {
  ...
  async getListClients(@Body() getListClientsDTO: getClientDTO) {
    return await this._clientService.getListClients(getListClientsDTO);
  }
  ...
  @ApiResponse({ status: 404, description: 'Клиент или пользователи не найдены' })
  @ApiBody({ type: getUsersListClientDTO })
  async getUsersListClient(@Param('id') id: string, @Body() getUsersListClient: getUsersListClientDTO) {
    return await this._clientService.getUsersListClient({ ...getUsersListClient, clientId: id });
  }
  ...
  async getDocsClient(@Param('id') id: string, @Body() clientsDocs: getDocsClientDTO) {
    return await this._clientService.getClientDocs({ ...clientsDocs, clientId: id });
  }
  ...
  async getOrdersClient(@Param('id') id: string, @Body() clientOrders: getOrdersClientDTO) {
    return await this._clientService.getOrdersClient({ ...clientOrders, clientId: id });
  }

  @Post('orders/docs/:orderId')
  @ApiOperation({ summary: 'Получение документов заказа клиента' })
  @ApiResponse({ status: 200, description: 'Документы заказа успешно получены' })
  @ApiResponse({ status: 404, description: 'Заказ или документы не найдены' })
  @ApiBody({ type: getOrderDocsClientDTO })
  async getOrderDocsClient(@Param('orderId') orderId: string, @Body() getOrderDocsClient: getOrderDocsClientDTO) {
    return await this._clientService.getOrderDocsClient({ ...getOrderDocsClient, orderId });
  }

  @Post()
  @ApiOperation({ summary: 'Создание нового клиента' })
  @ApiResponse({ status: 201, description: 'Клиент успешно создан' })
  @ApiResponse({ status: 409, description: 'Клиент с таким именем или ИНН уже существует' })
  @ApiBody({ type: CreateClientDTO })
  async createClient(@Body() createClient: CreateClientDTO) {
    return await this._clientService.createClient(createClient);
  }
  ...
  async createUserClient(@Param('id') clientId: string, @Body() createUserClient: CreateUserClientDTO) {
    return await this._clientService.createUserClient({ ...createUserClient, clientId });
  }
  ...
  async createOrderClient(@Param('id') clientId: string, @Body() createOrderClient: CreateOrderClientDTO) {
    return await this._clientService.createOrderClient({ ...createOrderClient, clientId });
  }
  ...
  async addDocsClient(@Param('id') clientId: string, @Body() addDocsClient: addDocsClientDTO) {
  ...
  async updateClient(@Param('id') id: string, @Body() updateClient: UpdateClientDTO) {
    return await this._clientService.updateClient({ ...updateClient, id });
  }
  ...
  @ApiResponse({ status: 404, description: 'Клиент или пользователь не найдены' })
  @ApiResponse({ status: 409, description: 'Пользователь с таким email уже существует' })
  @ApiBody({ type: UpdateUserClientDTO })
  async updateUserClient( @Body() updateUserClient: UpdateUserClientDTO) {
    return await this._clientService.updateUserClient({ ...updateUserClient });
  }
  ...
  async updateOrderClient(@Body() updateOrderClient: UpdateOrderClientDTO) {
    return await this._clientService.updateOrderClient({ ...updateOrderClient });
  }
}